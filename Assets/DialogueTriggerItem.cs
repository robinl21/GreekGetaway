...
public class DialogueTriggerItem : MonoBehaviour
{
    [Header("Visual Cue")]
    ...
    [SerializeField] private GameObject parentItem; // header object
    ...
    public void PickUpObjectP1() {
        IInventoryItem item = parentItem.GetComponent<IInventoryItem>();
        inventory1 = Inventory.inventory;
        inventory1.AddItem(item);
    }

    public void PickUpObjectP2() {
        IInventoryItem item = parentItem.GetComponent<IInventoryItem>();
            ...
            {   
                if (player.CompareTag("Player1")) {
                    if (InputManager.GetInstance().GetInteractPressed() && !DialogueManager.GetInstance().dialogueIsPlaying1) {
                        ...
                        Debug.Log("IN ITEM");
                        DialogueManager.GetInstance().EnterDialogueMode(inkJSON, true, PickUpObjectP1);
                ...
                if (player.CompareTag("Player2")) {
                    if (InputManager1.GetInstance().GetInteractPressed() && !DialogueManager.GetInstance().dialogueIsPlaying2) {
                        ...
                        DialogueManager.GetInstance().EnterDialogueMode(inkJSON, false, PickUpObjectP2); // player 2
